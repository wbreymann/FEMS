...
(yc_flat <- FlatCurve2(0.02500017, "2013-12-31"))
...
# (a)
(yc_flat1 <- FlatCurve2(0.04, "2000-12-31"))
...
# (b)
yc_flat1 <- FlatCurve2(0.03, "2000-12-31")
yc_flat2 <- FlatCurve2(0.05, "2010-12-31")
yc_flat3 <- FlatCurve2(0.04, "2015-12-31")
...
# (c)
(sol_2.c <- solve(method="compound", period=20, capital_0=sol_2.b$Value, capital_t=12500))
...
(bc3.1 <- BaseContract(Dates=c("2010-12-31"),
                       CashFlows=c(23000)))
(yc_flat <- FlatCurve2(0.025, "2010-12-31"))
...
########################################################################
# Ex 4
(yc_flat <- FlatCurve2(0.03, "2013-12-31"))
...
(bc5.1 <- BaseContract(Dates=c("2010-12-31"),
                       CashFlows=c(10000)))
yc_flat5.1 <- FlatCurve2(0.02, "2010-12-31")
yc_flat5.2 <- FlatCurve2(0.035, "2015-12-31")
...
(bc6.1 <- BaseContract(Dates=c("2010-12-31"),
                       CashFlows=c(100000)))
(yc_flat <- FlatCurve2(0.035, "2010-12-31"))
...
(sol_6.e1 <- solve(method="compound", period=15, capital_0=100000, capital_t=val_1$Value))


########################################################################
# Ex 7
(bc7.1 <- BaseContract(Dates=c("2029-01-01"), CashFlows=40000))
(yc_flat <- FlatCurve2(0.025, "2020-01-01"))

f = function(invest) {
  value(bc7.1, by="2020-01-01", curve=yc_flat, compound="compound")[1,2] - invest
}
...
(yc_flat <- FlatCurve2(0.032, "2020-01-01"))
f = function(tt) {
  by = as.Date("2020-01-01") + tt*365
  value(bc8.1, by=as.character(by), curve=yc_flat, compound="compound")[1,2] - 1500
}
